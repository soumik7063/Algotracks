...
const AtcoderContest = () => {
    ...
    // Effect to process contestData when it changes
    const now = Math.floor(Date.now() / 1000);
    useEffect(() => {
        if (contestData) {
            setPastContest(contestData.filter(contest => contest.start_epoch_second + contest.duration_second < now));
            setUpcomingContest(contestData.filter(contest => contest.start_epoch_second > now));
        }
    }, [contestData]);
    ...
    // Initialize bookmarks when upcoming contests are loaded
    useEffect(() => {
        if (upcomingContest && upcomingContest.length > 0) {
            // Initialize bookmarks from localStorage or set defaults
            const storedBookmarks = localStorage.getItem('contestBookmarks');
            if (storedBookmarks) {
                setBookmarked(JSON.parse(storedBookmarks));
            } else {
                // If you want some default bookmarks
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-indigo-700 py-6 px-6">
                    <h1 className="text-white text-2xl md:text-3xl font-bold text-center">Atcoder Contest Calendar</h1>
                                ...
                                {upcomingContest && upcomingContest.length > 0 ? (
                                    <div className="grid gap-6">
                                        {upcomingContest.slice().reverse().map(contest => {
                                            ...
                                            const hours = Math.floor(contest.duration_second / 3600);
                                            const minutes = Math.floor((contest.duration_second % 3600) / 60);
                                            const timeRemaining = getTimeRemaining(contest.start_epoch_second);
                                                            ...
                                                            <div className='flex justify-between'>
                                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 w-full">
                                                                    <div>
                                                                        ...
                                                                        <span className="font-medium">{formatDate(contest.start_epoch_second)}</span>
                                                                    </div>
                                                        ...
                                                        <div className="p-4">
                                                            ...
                                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-2 text-sm">
                                                                <div className="text-gray-500">
                                                                    <span className="font-medium">Date:</span> {formatDate(contest.start_epoch_second)}
                                                                </div>
                                                                <div className="text-gray-500">
