...
import {getUserProfile,getRecentSubmissions,getUpcomingContests,getPastContests} from '../../services/Leetcode'
...
const LeetcodeContest = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            setPastContest(pastContestdata.data);
            const upcontestData = await getUpcomingContests()
            setUpcomingContest(upcontestData.data);
        } catch (err) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-indigo-700 py-6 px-6">
                    <h1 className="text-white text-2xl md:text-3xl font-bold text-center">Leetcode Contest Calendar</h1>
                            ...
                            <>
                                ...
                                {upcomingContest && upcomingContest.length > 0 ? (
                                    <div className="grid gap-6">
                                        {upcomingContest.slice().reverse().map((contest,idx) => { 
                                            ...
                                            <h1 className='text-black'>asdkjfaksdjf</h1>
                                        })}
                                    </div>
                                ) : (
                                ...
                                {pastContest && pastContest.length > 0 ? (
                                    <>
                                        <div className="grid gap-4">
                                            {pastContest.slice(0, displayCount).map((contest,idx) => {
                                                const startDate = new Date(contest.startTime * 1000);
                                                const hours = Math.floor(contest.duration / 3600);
                                                const minutes = Math.floor((contest.duration % 3600) / 60);
                                                
                                                return (
                                                    <div key={idx} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-sm transition-shadow">
                                                        <div className="p-4">
                                                            ...
                                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-2 text-sm">
                                                                <div className="text-gray-500">
                                                                    <span className="font-medium">Date:</span> {formatDate(contest.startTime)}
                                                                </div>
                                                                <div className="text-gray-500">
            ...
            <div className="mt-4 text-center text-sm text-gray-500">
                Data provided by Leetcode API. Last updated: {new Date().toLocaleString()}
            </div>
        </div>
    );
};
